...
public class RubisUtilityFunction extends GenericUtilityFunction {
	
	private RubisModelHelper m_modelHelper = null;
	
	RubisUtilityFunction(AcmeModelInstance model, RubisModelHelper rubisModel) {
		m_modelHelper = rubisModel;
	}
	...
	public double getGenAdditiveUtility(GenericConfiguration config, GenericEnvironment env, int time) {
		double predictedResponseTime = responseTime(
	            ...
	            env.getDouble(RubisModelHelper.ENV_ARRIVAL_RATE_MEAN),
	            env.getDouble(RubisModelHelper.ENV_ARRIVAL_RATE_VARIANCE),
